...
function setupAdminRoutes(app) {
  ...
  authRoutes.post('/logout', authMiddleware('VIP-0', 'VIP-1', 'VIP-2', 'VIP-3', 'Admin'), authController.logout);
  ...
  adminRoutes.get('/payment-history', adminController.getPaymentHistoryForAdmin);
  adminRoutes.get('/payment-history/:user_id', adminController.getPaymentHistoryOfAUserByAdmin);
  app.use('/api/v1/admin', adminRoutes);

  // Payment Routes
  const paymentRoutes = express.Router();
  paymentRoutes.post('/vip-upgrade', authMiddleware('VIP-0', 'VIP-1', 'VIP-2'), paymentController.createVIPPayment);
